(function ( angular ) {
  angular.module( 'filterApp', [] )
    ...
    .filter( 'greaterThan', function () {
      return function ( input, fobj, fields ) {
        if ( !angular.isDefined( fobj ) ) {
          return input;
        }
        ...
        RECORDS: for ( var i = 0; i < input.length; i++ ) {
          record = input[i];
          ...
          for ( var f = 0; f < fields.length; f++ ) {
            ...
            if ( fobj[field]
              && record[field]
              && (Number( record[field] ) < Number( fobj[field] )) ) {
              continue RECORDS;
            }
          }
      ...
      return function ( input, fobj, fields ) {
        ...
        RECORDS: for ( var i = 0; i < input.length; i++ ) {
          ...
          for ( var f = 0; f < fields.length; f++ ) {
            ...
            if ( fobj[field]
              && record[field]
              && (!record[field].toUpperCase().startsWith( fobj[field].toUpperCase() )) ) {
              continue RECORDS;
            }
          }
    ...
    .controller( 'StateSearchCtrl', ['$scope',
      function ( $scope ) {
        $scope.changeSearch = function ( filterObj ) {
          // Bit of a cheat here. Might be a good use case for controller-as
          // syntax!
    ...
    .controller( 'StateCtrl', ['$scope', '$log', '$http', 'filterFilter',
      function ( $scope, $log, $http, filterFilter ) {
        ...
        $scope.selectState = function ( event ) {
          ...
          var selectedState = filterFilter( $scope.states,
            { name : searchState } )[0];
          $scope.$broadcast( 'selectedState', selectedState );
        };

        $scope.$on( 'nextState', function ( event, state, dir ) {
          var nextPos = Math.min(
            Math.max( ($scope.states.indexOf( state ) + dir), 0 ),
            $scope.states.length );
