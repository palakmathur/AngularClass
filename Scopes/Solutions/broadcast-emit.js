  ...
  angular.module( 'eventApp', [] )
    ...
    .controller( 'DetailCtrl', ['$scope', function ( $scope ) {
      ...
      $scope.nextPrev = function ( currentState, dir ) {
        ...
        $scope.$emit( 'nextState', currentState, dir );
      }
    ...
    .controller( 'MainCtrl', ['$scope', '$log', '$http', function ( $scope, $log, $http ) {
      ...
      $scope.$on( 'nextState', function ( event, currentState, dir ) {
        var currentPos = $scope.states.indexOf( currentState );
        var updatedPos = Math.min( Math.max( currentPos + dir, 0 ), $scope.states.length - 1 );
        var nextState = $scope.states[updatedPos];

        $scope.selectState( nextState );
      } );
