var frisby = require( 'frisby' ),
  baseURL = 'http://localhost:8001/doughnutShoppe/',
  configURL = baseURL + 'config';

frisby.create( 'Doughnut basic test' )
  .get( baseURL )
  .expectStatus( 200 )
...
frisby.create( 'DoughnutConfig basic test' )
  .get( configURL )
  .expectStatus( 200 )
  .expectHeaderContains( 'content-type', 'application/json' )
  .expectJSON( '0', {
...
frisby.create('Employee Update')
  .put(configURL + '/base', {
    values: ['cake', 'raised', 'gluten-free']
  })
  .expectStatus( 200 )
  .expectHeaderContains( 'content-type', 'application/json' )
  .expectJSON({message : 'Doughnut Config property base updated'})
  .toss();
/*
